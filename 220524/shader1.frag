...
void main(){
  ...
  const float quality = 25.0; // BLUR QUALITY (Default 4.0 - More is better but slower)
  float size = rad*23.0+2.0; //15
  vec2 radius = size/u_rez.xy;
  float cnt = 0.0;
  for( float d=0.0; d<pi; d+=pi/directions){
		for(float i=1.0/quality; i<=1.0; i+=1.0/quality)
        {
		  tartex = texture2D(tex0, uv+vec2(cos(d),sin(d))*radius*i);
      mult = ((tartex.x+tartex.y+tartex.z)/3.0)-((mytex.x+mytex.y+mytex.z)/3.0);
      //mult = sin(mult*2.0)*mult;
  ...
  //uv += dir2*10.0 + grad*-1.0; //-5.5 .2
  uv += grad* (rad* -5.0 + 6.0)* (strength* 4.0 + .25) * dist * press; //-5.5 .2
